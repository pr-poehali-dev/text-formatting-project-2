  ...
  return (
    <article className="bg-background text-foreground min-h-screen pb-24">
      ...
      <section className="py-16 animate-fade-up">
        <div className="article-container space-y-6 font-sans text-base md:text-lg leading-relaxed text-foreground">
          <p>
            Мария, основатель «Вашего персонального бухгалтера», знает это не из
            учебников.
          </p>

          <p>
            Она 26 лет отработала главным бухгалтером. Прошла через
            Новороссийский морской торговый порт, через строительство торговых
            центров, через олимпийские объекты в Сочи.
          </p>

          <p>
            Она видела, как крупные строительные компании теряют миллионы из-за
            банальных ошибок в документообороте. Как блокируются счета из-за
            того, что кто-то не проверил контрагента. Как срываются сроки сдачи
            объектов из-за кассовых разрывов, которые можно было предсказать за
            два месяца.
          </p>

          <p>
            И она видела, как это можно предотвратить. Если перестать работать «по
            факту» и начать работать на опережение.
          </p>

          <p>
            Именно поэтому 11 лет назад она создала компанию, которая работает
            по другим правилам.
          </p>
        </div>
      </section>
